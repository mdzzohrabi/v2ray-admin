...
const { writeFile, readFile, copyFile, appendFile, stat, open, watch, mkdir } = require('fs/promises');
...
function createCollectionManager(path, options = {}) {
    ...
    let { throwError = true, errorMesage } = options;
     ...
     * @returns {Promise<T?>}
     */
    return async function collection(key, value = null) {
        let mode = arguments.length == 2 ? 'read' : 'write';
        try {
            // Resolve collection directory
            let resolvedCollectionDir = resolve(path);

            // Create collection directory if not exists
            if (!existsAsync(resolvedCollectionDir))
                await mkdir(resolvedCollectionDir, { recursive: true });

            // Resovel collection file
            let collectionFile = resolve(join(dbDir(), key)) + '.json';

            // Write to collection
            if (mode == 'write') {
                await writeFile(collectionFile, JSON.stringify(value));
                return value;
            } else {
                // Read collection
                if (await existsAsync(collectionFile)) {
            ...
            log(errorMesage ?? `Error ${mode} to collection "${key}" in "${path}"`);
            if (throwError)
                throw err;
            return null;
        }
...
const cache = createCollectionManager(cacheDir(), { throwError: false });
 ...
 * @param {any}     value
 */
const db = createCollectionManager(dbDir(), { throwError: true });

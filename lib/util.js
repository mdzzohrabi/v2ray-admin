...
async function cache(key, value = undefined) {
    try {
        let resolvedCacheDir = resolve(cacheDir());
        if (!existsAsync(resolvedCacheDir))
        ...
        log(`Error reading cache key "${key}": ${err.message}`);
        return null;
    }
}
 ...
 * @param {any}     value
 */
async function db(key, value = undefined) {
    try {
        let resolvedDbDir = resolve(dbDir());
        if (!existsAsync(resolvedDbDir))
            await mkdir(resolvedDbDir, { recursive: true });

        let dbPath = resolve(join(dbDir(), key)) + '.json';
        if (typeof value !== 'undefined') {
            await writeFile(dbPath, JSON.stringify(value));
        } else {
            if (await existsAsync(dbPath)) {
                return JSON.parse((await readFile(dbPath)).toString('utf-8'));
            }
            return null;
        }
    } catch (err) {
        log(`Error reading db collection "${key}": ${err.message}`)
        throw err;
    }
}

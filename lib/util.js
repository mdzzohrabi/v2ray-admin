...
async function getUserConfig(user, tag) {
    ...
    inboundConfigs.forEach(config => {
        ...
        if (inbound?.protocol == 'vless') {
            strConfig = (`vless://${clientConfig.id}@${clientConfig.add}:${clientConfig.port}?path=%2F&host=${clientConfig.host}&security=${inbound?.streamSettings?.security=='tls'?'tls':'none'}&encryption=${inbound?.settings?.decryption ?? 'none'}&headerType=${clientConfig.type}&type=${clientConfig.net}&alpn=${clientConfig.alpn}&sni=${clientConfig?.sni}#${configName}`);
        } else {

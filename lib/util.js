...
async function getUserConfig(user, tag) {
    ...
    let serverAddress = inbound?.address ?? env.V2RAY_SERVER ?? 'n1.inetwork.pw';
    let configPrefix = inbound?.configPrefix ?? (`VIP-${inbound?.streamSettings?.network ?? 'tcp'}-${inbound?.streamSettings?.security == 'tls' ? '-tls-' : ''}`);
    ...
    let clientConfig = {
        ...
        "host": inbound?.configHost ?? "google.com",
        "id": user.id,
    ...
    if (inbound?.protocol == 'vless') {
        strClientConfig = `vless://${user.id}@${serverAddress}:${inbound.port}?path=%2F&host=${inbound?.configHost ?? 'google.com'}&security=${inbound?.streamSettings?.security=='tls'?'tls':'none'}&encryption=${inbound?.settings?.decryption ?? 'none'}&headerType=${inbound?.streamSettings?.tcpSettings?.header?.type}&type=${inbound?.streamSettings?.network}&sni=${serverAddress}#${configName}`        
    }

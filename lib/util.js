...
async function getUserConfig(user, tag) {
    ...
    let configPrefix = `VIP-${inbound?.streamSettings?.network ?? 'tcp'}-${inbound?.streamSettings?.security == 'tls' ? '-tls-' : ''}`;
...
function ping(url) {
    return new Promise((done, fail) => {
        let { hostname, port } = require('url').parse(url);
        if (!hostname || !port) return fail(Error('Invalid URL'));

...
async function getUserConfig(user, tag) {
    ...
    if (inbound?.protocol == 'vless') {
        strClientConfig = `vless://${user.id}@${serverAddress}:${inbound.port}?security=${inbound?.streamSettings?.security=='tls'?'tls':'none'}&encryption=${inbound?.settings?.decryption ?? 'none'}&headerType=${inbound?.streamSettings?.tcpSettings?.header?.type}&type=${inbound?.streamSettings?.network}#${configName}`        
    }

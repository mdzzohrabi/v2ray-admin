...
async function getUserConfig(user, tag) {
    ...
    if (inbound?.protocol == 'vless') {
        strClientConfig = `vless://${user.id}@${serverAddress}:${inbound.port}?path=%2F&security=${inbound?.streamSettings?.security=='tls'?'tls':'none'}&encryption=${inbound?.settings?.decryption ?? 'none'}&headerType=${inbound?.streamSettings?.tcpSettings?.header?.type}&type=${inbound?.streamSettings?.network}&sni=${serverAddress}#${configName}`        
    }

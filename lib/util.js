...
function getPaths() {
    ...
    return {
        configPath: env.V2RAY_CONFIG ?? '/usr/local/etc/xray/config.json',
        accessLogPath: env.V2RAY_ACCESS_LOG ?? '/var/log/xray/access.log',
        errorLogPath: env.V2RAY_ERROR_LOG ?? '/var/log/xray/error.log',
        v2ray: env.V2RAY ? env.V2RAY?.match(/\/|\\/) ? resolve(env.V2RAY) : env.V2RAY : 'xray'
    }
}
 ...
 * @returns {import('../types').V2RayConfig}
 */
 ...
 * @returns {Promise<import('../types').UserUsages>}
 */
...
function parseLogLine(line) {
    if (line.includes('>>') || line.includes('->')) {
        // XRay Format
        let [date, time, clientAddress, status, destination, inbound, _, outbound, email, user] = line.split(' ').map(x => x.replace(/^\[|\]$/, ''))
        if (!user) return null;
        user = user.trim();
        let dateTime = new Date(date + ' ' + time);
        return {date, time, clientAddress, status, destination, inbound, outbound, route: outbound, email, user, dateTime};
    } else {
        // V2Ray Format
        let [date, time, clientAddress, status, destination, outbound, email, user] = line.split(' ').map(x => x.replace(/^\[|\]$/, ''));
        if (!user) return null;
        user = user.trim();
        let dateTime = new Date(date + ' ' + time);
        return {date, time, clientAddress, status, destination, outbound, route: outbound, email, user, dateTime};
    }
}
 ...
 * @param {import('../types').V2RayConfig} config Config
 * @param {string} email Email
 * @param {string?} tag Tag
 * @returns {import('../types').V2RayConfigInboundClient | undefined}
 */
 ...
 * @param {string?} tag Tag
 * @param {string} email Email
 * @returns {import('../types').V2RayConfigInboundClient[]}
 */
 ...
 * @param {import('../types').V2RayConfig} config Config
 * @param {string} email Email
 * @returns {import('../types').V2RayConfigInbound[]}
 */
 ...
 * @param {string?} tag Tag
 * @param {Partial<import('../types').V2RayConfigInboundClient>?} userInfos User information
 * @returns {Promise<import('../types').V2RayConfigInboundClient>}
 */
async function addUser(configPath, email, protocol, tag = null, userInfos = null) {
    ...
    let inbound = config.inbounds?.find(x => (!protocol || x.protocol == protocol) && (!tag || x.tag == tag));
    ...
    if (inbound.streamSettings?.security == 'reality' && !user.flow) {
 ...
 * @param {import('../types').V2RayConfigInboundClient} user Client
 * @param {string} tag
 */
async function getUserConfig(user, tag) {
    ...
    // @ts-ignore
    user = inbound?.settings?.clients?.find(x => x.id == user.id);
    ...
    inboundConfigs.forEach(config => {
        ...
        let clientConfig = {
            ...
            "ps": configName,
            "scy": "auto",
            "sni": config?.sni ?? (inbound?.streamSettings?.realitySettings?.serverNames ?? [])[0] ?? "",
        ...
        if (inbound?.protocol == 'vless') {
            let vlessOptions = {
                ...
                headerType: clientConfig?.type,
                type: clientConfig?.net,
                alpn: clientConfig?.alpn,
                sni: clientConfig?.sni,
                pbk: inbound?.streamSettings?.realitySettings?.publicKey,
                flow: user?.flow,
                sid: (inbound?.streamSettings?.realitySettings?.shortIds ?? [])[0],
                fp: inbound?.streamSettings?.security == 'reality' ? 'chrome' : undefined
            };

            strConfig = (`vless://${clientConfig.id}@${clientConfig.add}:${clientConfig.port}?${Object.keys(vlessOptions).filter(k => vlessOptions[k]).map(k => `${k}=${vlessOptions[k]}`).join('&')}#${configName}`);
        } else {
...
/**
 * @param {import('../types').V2RayConfig} config
 * @param {string} user User email
 * @param {string} outboundTag Route outbound tag name
 */
function addUserToRoute(config, user, outboundTag) {
    ...
    if (routeRule) {
        if (Array.isArray(routeRule.user)) {
            if (!routeRule.user.includes(user))
                routeRule.user.push(user);
        }
        else
            routeRule.user = [user];
    }
 ...
 * @param {string} outboundTag Route outbound tag name
 */
function removeUserFromRoute(config, user, outboundTag) {
    // Routing rule to add user
    let routeRule = config?.routing?.rules?.find(x => x.outboundTag == outboundTag);
    if (routeRule) {
        let index = routeRule.user?.indexOf(user) ?? -1;
        if (index >= 0) {
...
async function restartService() {
    ...
    let {stderr, stdout} = await execAsync('systemctl restart xray');
 ...
 * @param {import('../types').Change[]} changes Changes
 */
...
/**
     * 
     * @param {import('../types').ServerNode} serverNode 
     * @param {string} action 
     * @param {string} method 
     * @param {any} body 

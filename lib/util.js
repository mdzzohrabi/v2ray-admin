...
async function getUserConfig(user, tag) {
    ...
    let clientConfigs = inbound?.clientConfigs ?? [];

    if (clientConfigs.length == 0) clientConfigs.push({});
    let clientConfig = {};
    let strConfigs = [];
    ...
    clientConfigs.forEach(config => {
        ...
        if (inbound?.protocol == 'vless') {
            strConfigs.push(`vless://${clientConfig.id}@${clientConfig.add}:${clientConfig.port}?path=%2F&host=${clientConfig.host}&security=${inbound?.streamSettings?.security=='tls'?'tls':'none'}&encryption=${inbound?.settings?.decryption ?? 'none'}&headerType=${clientConfig.type}&type=${clientConfig.net}&alpn=${clientConfig.alpn}&sni=${serverAddress}#${configName}`);
        } else {
            strConfigs.push(`${inbound?.protocol}://${Buffer.from(JSON.stringify(clientConfig)).toString('base64')}`);
        }
    });

    return {clientConfig, strClientConfig: strConfigs.join('\n')};
}

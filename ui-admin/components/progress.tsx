        ...
        <div className="flex flex-row bg-gray-200 h-6 w-full relative rounded-xl">
            {!bars || bars.length == 0 ? <div className="flex-1 flex items-center justify-center">{noBarsMessage}</div> : <>
                {bars.map((x, index) => {
                    ...
                    return <Popup popup={<>{x.title}  ({percent.toFixed(2)}%)</>} className={classNames("items-center justify-center flex text-white text-[.6rem] first:rounded-tl-xl first:rounded-bl-xl last:rounded-tr-xl last:rounded-br-xl whitespace-nowrap", x.className ?? colors[index])} style={{ width: `${percent}%` }}>
